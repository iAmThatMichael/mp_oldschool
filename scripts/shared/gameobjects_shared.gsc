...
REGISTER_SYSTEM( "gameobjects", &__init__, undefined )
...
function set_dropped()
{
	...
	trace_size = 10;
	trace = PhysicsTrace( startOrigin, endOrigin, ( -trace_size, -trace_size, -trace_size ), ( trace_size, trace_size, trace_size ), self, PHYSICS_TRACE_MASK_PLAYER );
...
function should_be_reset( minZ, maxZ, testHurtTriggers )
{
	...
	if ( IS_TRUE( testHurtTriggers ) && self.visuals[0] is_touching_any_trigger_key_value( "trigger_hurt", "classname", minZ, maxZ ) )
		return true;
...
function wait_take_carry_weapon( weapon )
{
	...
	wait( Max( 0, weapon.fireTime - ( 2 * SERVER_FRAME ) ) );
...
function track_carrier(object)
{
	...
	WAIT_SERVER_FRAME; // wait a frame so the carrier gets set
...
function prox_trigger_think()
{
	...
	DEFAULT( self.trigger.remote_control_player_can_trigger, false );
...
function make_solid()
{
	...
	while(1)
	{
		// TODO: O.S. hack needed?
		players = ( isdefined( level.players ) ? level.players : GetPlayers() );
...
function get_objective_ids( str_team )  // self = gameobject
{
	...
	if ( IS_TRUE( self.newstyle ) )
	{
		// newStyle objectives only use one objective index
		ARRAY_ADD( a_objective_ids, self.objectiveID );
	}
	else
	{
		...
		for ( i = 0; i < a_keys.size; i++ )
		{
			...
			{
				ARRAY_ADD( a_objective_ids, self.objID[ a_keys[ i ] ] );
			}
		}
...
#/
	...
	if ( nextId > HIGHEST_OBJECTIVE_ID )
	{
		nextId = HIGHEST_OBJECTIVE_ID;
	}
...
function release_obj_id( objID )
{
	...
	{
		if ( objID == level.releasedObjectives[i] && objID == HIGHEST_OBJECTIVE_ID )
		{
			return;
		}
/#
...
function give_pack_object( object )
{
	...
	if ( !object.newStyle )
	{
		if ( isdefined( object.packIcon ))
		{
			if ( self IsSplitscreen() )
			{
				elem = hud::createIcon( object.packIcon, SPLIT_SIZE, SPLIT_SIZE );
				elem.y = -90;
			...
			{
				elem = hud::createIcon( object.packIcon, STANDARD_SIZE, STANDARD_SIZE );
				elem.y = PACK_ICON_Y;
